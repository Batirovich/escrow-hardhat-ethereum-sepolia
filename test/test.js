...
describe('Escrow', function () {
  ...
  beforeEach(async () => {
    ...
    broker = ethers.provider.getSigner(2);
  ...
  describe('after approval from address other than the broker', () => {
    it('should revert', async () => {
      await expect(contract.connect(beneficiary).approve()).to.be.reverted;
    });
  });

  describe('after approval from the broker', () => {
    it('should transfer balance to beneficiary', async () => {
      const before = await ethers.provider.getBalance(beneficiary.getAddress());
      const approveTxn = await contract.connect(broker).approve();
